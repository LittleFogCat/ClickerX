...
import top.littlefogcat.clickerx.core.Core.handlerCompat.innerH
...
import top.littlefogcat.common.utils.LuaUtils
import top.littlefogcat.common.utils.equalsOrNull
...
object Core : LuaJavaInterface {
    ...
    /**
     * Current executing script, `null` if no script is running.
     *
     * Assigned when a script executed by [runScript], and be `null` again when the execution done.
     */
    internal var runningScript: Script? = null

    /**
     * This event will be passed via EventBus from who initialize the [Core].
     */
    @Subscribe
    fun onNotifiedInitial(event: CoreInitEvent) {
        init(event.screenWidth, event.screenHeight)
    }

    /**
     * Initialization function. Mainly load the **`init.lua`** as basic library.
     */
    fun init(screenWidth: Int, screenHeight: Int) {
        Log.d(TAG, "Core init")
        // 获取屏幕尺寸
        screenSize.widthPixels = screenWidth
        screenSize.heightPixels = screenHeight

        coroutine.launch {
            globals.finder = ResourceFinder { filename ->
                AppContext.get().assets.open(filename)
            }
            LOCK.withLock {
                try {
                    // 加载初始化脚本
            ...
            // 加载运行本地脚本
            val localConfigs = repo.loadLocalScripts()
     ...
     * Execute [script].
     *
     * While executing, [runningScript] will be set to [script] and back to `null` after the execution done.
     * This function is suspend so it can only be invoke in a coroutine.
     */
    suspend fun runScript(script: Script) {
    ...
    override fun setTrigger(name: String, triggerType: Int, args: LuaTable, action: LuaClosure) {
        val service = service ?: throw LuaException(NULL_SERVICE, "setTrigger")
