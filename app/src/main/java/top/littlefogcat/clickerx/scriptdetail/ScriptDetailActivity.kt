 ...
 *
 * 在启动时通过ARouter注入[script]对象，并将其赋给[ScriptDetailViewModel.config]，用于整个配置详情功能模块共享的config实例对象。
 */
...
class ScriptDetailActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        getViewModel(ScriptDetailViewModel::class.java).config.value = script // 通过ViewModel保存config实例
    }
